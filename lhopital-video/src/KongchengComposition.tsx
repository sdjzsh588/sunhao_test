...
import { PaperBackground } from "./yeshi/ink";
import {
  SCrisis,
  SDecision,
  SEmptyCity,
...
const SCENES = [STitle, SCrisis, SDecision, SEmptyCity, SRetreat, SOutro];

// Floors (frames). Scenes grow to fit narration but never drop below these, so
// the animations always have room.
const SCENE_FLOOR = [300, 360, 330, 360, 420, 360];

const HEAD_PAD = 16; // lead-in before a scene's narration starts
const TAIL_PAD = 28; // breathing room after narration ends

// Crossfades throughout suit the ink-wash mood.
const PRESENTATIONS = [fade(), fade(), fade(), fade(), fade()];

// Prefer a guqin/古风 track if present, then the generic MiniMax bed.
...
export const KongchengComposition: React.FC<Props> = ({
  sceneDurations,
  voiceoverFiles,
  bgmFile,
}) => {
