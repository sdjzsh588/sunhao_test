// Generates per-scene Chinese narration MP3s for the Vieta video with MiniMax
// T2A (native Mandarin voices). Mirrors generate-voiceover-minimax.ts but reads
// the Vieta script and writes to public/voiceover/vieta/.
//
//   export MINIMAX_API_KEY=...
//   export MINIMAX_GROUP_ID=...
//   # optional: export MINIMAX_VOICE_ID="Chinese (Mandarin)_Warm_Bestie"
//   node --experimental-strip-types generate-voiceover-vieta-minimax.ts
//   npx remotion render Vieta out/vieta.mp4
import { mkdirSync, writeFileSync } from "node:fs";
import { dirname } from "node:path";
import { VIETA_VOICEOVER, vietaVoiceoverPath } from "./src/vieta/script.ts";
...
for (const line of VIETA_VOICEOVER) {
  ...
  const out = `public/${vietaVoiceoverPath(line.id, "mp3")}`;
  mkdirSync(dirname(out), { recursive: true });
  writeFileSync(out, audio);
  console.log(`saved ${out} (${(audio.length / 1024).toFixed(0)} KB)`);
}

console.log("\nDone. Render with: npx remotion render Vieta out/vieta.mp4");
